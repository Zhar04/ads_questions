...
- **Недавно показанные** (анти-повтор): `getSeen/pushSeen(ids,subject,limit)` — скользящее окно id (лимит ≈ 45% банка,
  новые в конце). `quiz.js` исключает их из выборки blitz/full/topic (через `opts.avoid`) и дописывает показанные обратно.
