...
- `quiz-engine.js` — чистая логика (без DOM, тестируемая): `shuffle`, `stratifiedSample`, `topicSample`,
  `blockSet`, `allUsable` (весь банк — для мега-теста), `isCorrect`, `countMistakes`, `questionPoints`,
  `scoreQuiz`, `createTimer`, `fmtTime`, `shuffleQuestion` (перемешивание вариантов с безопасным ремапом
  ключа и разбора). `FULL_TEST_MINUTES`. **Анти-повтор:** `stratifiedSample`/`topicSample` принимают
  `opts.avoid` (Set/массив id недавно показанных) — их стараются не брать (с откатом ко всему пулу, если не хватает).
  **`searchQuestions(questions, query, limit)`** — нечёткий поиск по банку (совпадение ключевых слов + общая
  подстрока, нормализация ё/степеней/регистра, стоп-слова); используется панелью поиска мега-теста и `tools/check-new-questions.mjs`.
...
- `progress.js` — прогресс в localStorage (раздельно по предметам): чтение блоков, лучший квиз/полный тест,
  **журнал ошибок**, **история результатов** и **окно недавно показанных** (`getSeen`/`pushSeen` — анти-повтор) (см. ниже).
...
- `tools/check-new-questions.mjs` — проверка находок на дубликаты (`node`, использует `searchQuestions`); читает `inbox/`.
- `inbox/` — локальная «входящая» для новых найденных вопросов; вердикт 🔴дубликат/🟡похоже/🟢новый. Содержимое в `.gitignore` (кроме README).
...
`mega` (**мега-тест** — весь банк подряд через `allUsable`: без таймера, разбор сразу как в блице, варианты НЕ перемешиваются;
внизу — **панель поиска по банку** (`installMegaSearch`/`searchQuestions`), крепится к родителю `#quiz` и переживает перерисовку вопроса).
...
- Применяется в обучающих режимах (blitz/topic/block/mistakes), **не** в `full` и `mega`
...
- **Недавно показанные** (анти-повтор): `getSeen/pushSeen(ids,subject,limit)` — скользящее окно id (лимит = размер банка
  − ~40, т.е. оставляем ~40 свежих; новые в конце). `quiz.js` исключает их из выборки blitz/full/topic (через `opts.avoid`)
  и дописывает показанные обратно — вопросы не повторяются несколько тестов подряд.
